...
resource "aws_security_group" "terraform-grey-sg" {
  egress = [{
...
resource "aws_lb_target_group" "terraform-grey-tg" {
  name        = "terraform-grey-tg"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_lb" "terraform-grey-lb" {
  name               = "terraform-grey-lb"
...
resource "aws_lb_listener" "terraform-grey-lbl" {
  load_balancer_arn = aws_lb.terraform-grey-lb.arn
  port              = 80
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.terraform-grey-tg.arn
  }
}
...
resource "aws_autoscaling_group" "terraform-grey-asg" {
  name                      = "terraform-grey-asg"
  vpc_zone_identifier       = aws_lb.terraform-grey-lb.subnets
  max_size                  = 10
  min_size                  = 2
  desired_capacity          = 2
  health_check_grace_period = 300
  health_check_type         = "ELB"
  target_group_arns         = [aws_lb_target_group.terraform-grey-tg.arn]
